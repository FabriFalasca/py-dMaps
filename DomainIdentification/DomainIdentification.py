...
class DomainIdentification(object):
    ...
    def domain_identification(self):
        ...
        while(True):  # while we can either expand or merge domains
            ...
            print('Iteration: '+str(iteration)+", expanded = " +
                  str(expanded)+", merged: "+str(merged))
            iteration += 1
            if(merged ==False and expanded==False):
                break
    ...
    def merge_domains(self):
        ...
        while(True):  # while we can merge domains...
            ...
            for i in range(len(self.domains)):
                for j in range(i+1, len(self.domains)):
                    if(self.does_overlap(self.domains[i], self.domains[j])):
                        ...
                        overlapping_domain.map = self.domains[i].map + \
                            self.domains[j].map
            ...
            for ov_domain in overlapping_domains:
                ov_domain.homogeneity = self.get_homogeneity_overlapping_domain(
                    ov_domain)
            ...
            if(best_ov_domain.homogeneity > self.delta):  # merge
                print('Overlapping domain found, homogeneity: ' +
                      str(best_ov_domain.homogeneity))
                ...
                # randomly assign one id
                new_domain = Domain(domain_a.domain_id)
                ...
                for grid_cell_id, grid_cell in domain_b.grid_cells.items():
                    new_domain.grid_cells[grid_cell_id] = grid_cell
                # update the map
                new_domain.map = np.zeros(
                    (domain_a.map.shape[0], domain_a.map.shape[1]))
    ...
    def expand_domains(self):
        ...
        start_merging = False
        # Changed by Fab
        while(start_merging ==  False):
            self.domains.sort(key=lambda x: x.homogeneity, reverse=True)
            # for each domain
            for i, domain in enumerate(self.domains):
                print('Expanding domain '+str(i))
                domain, expanded = self.expand_domain(domain)
                if(expanded):
                    # remove that domain from the list
                    self.domains.pop(i)
                    # add the new expanded domain at the same position
                    self.domains.insert(i,domain) #add by Fab
                    print('Expanded domain '+str(i))
                    """
    Main logic.
    if the domain is expanded try to see if we have any overlapping domains,
    if their union is larger
    than delta.
    ...
    """
                        ...
                        if(i != j):  # do not check with overlap with yourself
                            if(self.does_overlap(domain, a_domain)):
                                ...
                                if(self.get_homogeneity_union(domain, a_domain) > self.delta):
                                    print('Detected two domains that (a) overlap and (b) homogeneity > delta')
                                    start_merging = True
                                    break
                                else:
                                    continue
                ...
                if(start_merging == True): # Remove
                    break
            if(expanded == False): # Add else: break
                break
        return expanded, start_merging
    ...
    def does_overlap(self, domain_a, domain_b):
        ...
        if(len(overlaps) > 0):
            ...
            return False
    '''
    For a domain D, expand 1 cell at a time.
    A cell can be included in the domain if it's (a) a neighbor (lives on the domain's border)
    and (b) the homogeneity of the domain plus the cell is greater than delta.
    The domain expansion stops when those constraints are not satisfied.
    I found (Fabri) that in this way the algorithm gives more robust results than simply
    asking for merging after every single expansion. Sue me if needed.
    '''
    def expand_domain(self, domain):
        ...
        while(True): # keep expanding it while True
            ...
            for grid_cell_id, grid_cell in domain.grid_cells.items():
                ...
                coords = np.expand_dims(np.asarray(
                    [grid_cell.lat, grid_cell.lon]), 0)
                _, k_neighborhood_indices = self.nearest_neighbors.kneighbors(
                    coords, n_neighbors=8+1)
                k_neighborhood_indices = k_neighborhood_indices[0]
                # remoove self
                k_neighborhood_indices = k_neighborhood_indices[1:]
                #print('k neighborhood indices')
                #print(k_neighborhood_indices)

                for k_index in k_neighborhood_indices:
                    # x,y index of neighboring grid cell
                    neigh_indices = self.indices[k_index]
                    # get the corresponding grid cell
                    try:
                        # neighboring grid cell id
                        neigh_grid_cell_id = self.pos_to_grid_cell_id[str(
                            neigh_indices[0])+","+str(neigh_indices[1])]
                        candidate_grid_cell = self.grid_cells[neigh_grid_cell_id]
                        # grid cell not in domain = grid cell in border
                        if(candidate_grid_cell not in domain_grid_cells):
            ...
            for candidate_grid_cell in candidate_grid_cells:
                candidate_grid_cell_ts = self.data[:,
                                                   candidate_grid_cell.index[0], candidate_grid_cell.index[1]]
                score = 0.

                for domain_grid_cell in domain_grid_cells:
                    domain_grid_cell_ts = self.data[:,
                                                    domain_grid_cell.index[0], domain_grid_cell.index[1]]
            ...
            if(best_candidate.score > self.delta):  # then add the grid cell
                ...
                domain.homogeneity = (bc_score+current_homogeneity) / \
                    (len(domain.grid_cells)*(len(domain.grid_cells)+1)/2.)
                domain.grid_cells[best_candidate.id] = best_candidate
                # update domain map
                domain.map[best_candidate.index[0], best_candidate.index[1]] = 1
                # Check the dimensionality of the domain
                #print('# of grid cells in domain '+str(np.sum(domain.map.flatten())))
                is_expanded = True
                #it = it + 1
            else:  # this domain cannot be expanded
                break
        #return domain, is_expanded
        return domain, is_expanded
    ...
    def domain_initialization(self):
        ...
        for i in range(len(seed_locations[0])):  # 1 domain per seed location
            ...
            init_index = np.asarray(
                [seed_locations[0][i], seed_locations[1][i]])
            ...
            # corresponding lat,lon of init_index
            init_coords = self.coords[coord_index]
            ...
            for k_index in k_neighborhood_indices:
                ...
                # get the corresponding grid cell
                try:
                    grid_cell_id = self.pos_to_grid_cell_id[str(
                        neigh_indices[0])+","+str(neigh_indices[1])]
            ...
            for i in range(len(k_neighborhood_indices)):
                ...
                for j in range(i+1, len(k_neighborhood_indices)):
                    ...
                    corr = lagged_correlation(
                        timeseries_i, timeseries_j, tau=0, normed=True)
    ...
    def _init_grid_cells(self):
        ...
        # key str ("x,y") position, value: grid cell id.
        pos_to_grid_cell_id = {}
    ...
    def _init_nearest_neighbors(self, k):
        nearest_neighbors = NearestNeighbors(
            n_neighbors=k+1, algorithm='ball_tree', metric='haversine')
    ...
    def dump_output(self):
        ...
        for domain in self.domains:
            ...
            with open(self.output_dir+"info_"+str(domain.domain_id)+".txt", 'w') as f:
                f.write("Number_of_grid_cells:" +
                        str(len(domain.grid_cells))+"\r\n")
