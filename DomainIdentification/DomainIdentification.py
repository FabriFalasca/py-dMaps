...
class DomainIdentification(object):
	...
	def expand_domains(self):
		...
		while(start_merging == False):
			...
			expanded = False; # set it to False
			##for each domain
			for i, domain in enumerate(self.domains):
				domain, is_expanded = self.expand_domain(domain);
				if(is_expanded == True):
					print('Domain '+str(i)+' was expanded')
					expanded = True
					"""
                	Main logic.
						...
						if(i!= j): ##do not check with overlap with yourself
							if(self.does_overlap(domain, a_domain)):
								...
								if(self.get_homogeneity_union(domain,a_domain) > self.delta):
									print('Found 2 domains that can be merged. Lets merge them')
									start_merging = True;
									break;
								else:
									continue
